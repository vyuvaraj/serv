...
func handleIncidentAnalyze(w http.ResponseWriter, r *http.Request) {
	...
	case "servgate":
		...
		runbookSteps = []string{"1. Check route health metrics", "2. Run: serv gate restart --graceful", "3. Confirm service recovery logs"}
	case "servstore":
		suggestedRunbook = "rb-store-heal"
		runbookSteps = []string{"1. Assess filesystem status", "2. Run: serv store heal --shards=all", "3. Re-verify replica consistency"}
	default:
		suggestedRunbook = "rb-queue-purge"
		runbookSteps = []string{"1. Verify DLQ message count", "2. Run: serv queue purge dlq", "3. Auto-notify subscribers"}
	}
